...
public class ManagersDAO {
	...
	public Managers findById(java.lang.Integer id) {
		...
		try {
			Managers instance = (Managers) getCurrentSession().get(
					"Dao.Managers", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Managers instance) {
		...
		try {
			List results = getCurrentSession().createCriteria("Dao.Managers")
	...
	public Managers merge(Managers detachedInstance) {
		...
		try {
			Managers result = (Managers) getCurrentSession().merge(
					detachedInstance);
